...
| Profile Photo Preview Missing | When a user uploaded a new profile photo via the **“Upload New Picture”** button, the file was sent correctly but the new image was not displayed in the placeholder preview until saving the full profile. | Added a JavaScript `change` event handler to immediately update the `<img>` element’s `src` with a temporary preview (`URL.createObjectURL`) when a new file is selected. |
| Missing Download Links for Wishlist Items | Digital products added to the cart directly from the gallery correctly displayed download links after checkout, but products moved from the **wishlist to the shopping cart** did not show download links on the confirmation page (though they appeared in confirmation emails). | Fixed the `move_to_cart` view to add `"format": "digital"` to session bag items for products with a `file` field, ensuring consistency with normal “add to bag” logic so downloads are detected at checkout success. |
